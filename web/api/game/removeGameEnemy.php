...
if (!isset($post->id_game) || !isset($post->progress)) {
    $response[DATA][ACHIVIMENTS_PATH] = NULL;
...
foreach ($cursor as $row) {
    ...
    $totalParent->isBuilder3 = $row["isBuilder3"];
...
if ($id_child != null) {
    ...
    foreach ($cursor as $row) {
        ...
        $totalChild->isBuilder3 = $row["isBuilder3"];
...
if (strcmp($id_parent, $idUser) == 0) {
    ...
    $saveFull = $progress != TIME_OUT_PARENT;
    ...
    $setQuery = array(
        '$set' => array(
            ...
            "isPatient" => new MongoInt32($totalParent->isPatient += $resUpd["parent"]["isPatient"] && $saveFull ? 1 : 0),
    ...
    if ($id_child != null) {
        if (strcmp($id_child, $id_parent) != 0) {
            ...
            $saveFull = $progress != TIME_OUT_CHILD;
            ...
            $setQuery = array(
                '$set' => array(
                    ...
                    "isPatient" => new MongoInt32($totalChild->isPatient += $resUpd["child"]["isPatient"] && $saveFull ? 1 : 0),
